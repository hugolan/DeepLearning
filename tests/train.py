...
def train_model(load_model=False, save_model=False, model=None, optimizer=None, loss_fn=None, batch_size=100,
                num_epochs=1, input_dataset=None, dataset_swap=False):
    ...
    if load_model:
        ...
        model.train(train_input0, train_input1, optimizer=optimizer, loss_fn=loss_fn, num_epochs=num_epochs,
                    batch_size=batch_size, dataset_swap=dataset_swap)
        if save_model:
